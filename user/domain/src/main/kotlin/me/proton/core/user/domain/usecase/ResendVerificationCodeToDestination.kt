...
class ResendVerificationCodeToDestination @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        return when (tokenType) {
            UserVerificationTokenType.SMS -> userValidationRepository.sendVerificationCodePhoneNumber(
                sessionId = sessionId,
                phoneNumber = destination,
                type = tokenType
            )
            UserVerificationTokenType.EMAIL -> userValidationRepository.sendVerificationCodeEmailAddress(
                sessionId = sessionId,
