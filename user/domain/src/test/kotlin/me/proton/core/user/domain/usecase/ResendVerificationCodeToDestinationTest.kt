...
import me.proton.core.user.domain.exception.InvalidValidationOptionException
...
class ResendVerificationCodeToDestinationTest {
    ...
    fun `send verification token with token type email`() = runBlockingTest {
        ...
        val result = useCase.invoke(sessionId, UserVerificationTokenType.EMAIL, testEmail)
    ...
    fun `send verification token with token type sms`() = runBlockingTest {
        ...
        val result = useCase.invoke(sessionId, UserVerificationTokenType.SMS, testPhoneNumber)
        assertEquals(VerificationResult.Success, result)
    }
    ...
    fun `send verification token with token type invalid option`() = runBlockingTest {
        ...
        useCase.invoke(sessionId, UserVerificationTokenType.CAPTCHA, testPhoneNumber)
    }
}
