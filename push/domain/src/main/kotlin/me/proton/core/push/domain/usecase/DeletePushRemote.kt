...
public class DeletePushRemote @Inject constructor(
    private val pushRemoteDataSource: PushRemoteDataSource,
    private val eventManagerProvider: EventManagerProvider,
) {
    public suspend operator fun invoke(userId: UserId, pushId: PushId) {
        eventManagerProvider.suspend(EventManagerConfig.Core(userId)) {
            pushRemoteDataSource.deletePush(userId, pushId)
        }
    }
}
