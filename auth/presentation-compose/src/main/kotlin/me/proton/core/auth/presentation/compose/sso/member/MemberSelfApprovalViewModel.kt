...
import me.proton.core.auth.presentation.compose.sso.member.MemberApprovalAction.Close
import me.proton.core.auth.presentation.compose.sso.member.MemberApprovalAction.Confirm
import me.proton.core.auth.presentation.compose.sso.member.MemberApprovalAction.Load
import me.proton.core.auth.presentation.compose.sso.member.MemberApprovalAction.Reject
import me.proton.core.auth.presentation.compose.sso.member.MemberApprovalAction.ValidateCode
...
@HiltViewModel
public class MemberSelfApprovalViewModel @Inject constructor(
    ...
    public val state: StateFlow<MemberSelfApprovalState> = mutableAction.flatMapLatest { action ->
        when (action) {
            is Load -> onLoad()
            is Close -> onClose()
    ...
    private suspend fun getPendingDeviceId(): AuthDeviceId? =
        ...
        authDeviceRepository.getByUserId(userId, refresh = true)
            ...
            .filter { it.platform?.enum == AuthDevicePlatform.Android }
            .lastOrNull { it.state == AuthDeviceState.PendingActivation }?.deviceId
}
