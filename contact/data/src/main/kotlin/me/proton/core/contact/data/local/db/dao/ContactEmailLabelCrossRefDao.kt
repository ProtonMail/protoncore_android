...
@Dao
abstract class ContactEmailLabelCrossRefDao: BaseDao<ContactEmailLabelCrossRefEntity>() {
    @Transaction
    @Query("SELECT labelId FROM ContactEmailLabelCrossRefEntity WHERE contactEmailId = :contactEmailId")
    abstract fun getAllLabels(contactEmailId: ContactEmailId): Flow<List<String>>

    @Query("DELETE FROM ContactEmailLabelCrossRefEntity WHERE contactEmailId IN (:contactEmailIds)")
