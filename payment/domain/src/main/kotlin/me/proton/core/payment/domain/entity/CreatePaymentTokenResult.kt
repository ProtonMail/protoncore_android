...
sealed class PaymentToken(
    open val status: PaymentTokenStatus
) {
    data class CreatePaymentTokenResult(
    ...
    data class PaymentTokenStatusResult(
        override val status: PaymentTokenStatus
    ) : PaymentToken(status)
}

enum class PaymentTokenStatus(internal val id: Int) {
